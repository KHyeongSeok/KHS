...
namespace _250527_SocketServerConsole
{
    internal class Program
    {
        static ConcurrentBag<TcpClient> connectedClients = new ConcurrentBag<TcpClient>();
        // async : 비동기 - 빠름 ex) 10초, 100초 => 10 + 110 = 120
        // sync : 동기 ex) 100초, 10초 => 100 + 110 = 210
        static async Task Main(string[] args)
        {
            ...
            {
                server.Start();
                Console.WriteLine("서버가 실행됨(포트9000)");
                while (true)
                {
                    Console.WriteLine("클라이언트 기다려요");
                    TcpClient client = await server.AcceptTcpClientAsync();
                    connectedClients.Add(client);
                    Console.WriteLine($"클라이언트 연결 되었어요 {client.Client.RemoteEndPoint}");
        ...
        static async Task HandleClientAsync(TcpClient client)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"클라이언트 ({client.Client.RemoteEndPoint}) 연결 종료됨");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"클라리언트 처리 중 오류 : {ex.Message}");
            }
            finally
            {
                stream?.Close();
        ...
        static async Task BroadcastMessageAsync (string message, TcpClient excludedClient)
        {
            byte[] messageBytes = Encoding.UTF8.GetBytes(message);
            foreach(var client in connectedClients.Where(c => c.Connected && c != excludedClient))
            {
                try
                {
